...
from .utils_for_tests import text, t1, t2, t3, \
                    checkNumOutputs, checkHasOutput, checkDoesntHaveOutput
...
@pytest.mark.parametrize("grammar, expected_results", [
    ...
    (Seq(text("hello"), text("world")), 
        (("text", "helloworld"),)),
    # 3. Sequence text:hello+test:<empty>
    (Seq(text("hello"), text("")), 
        (("text", "hello"),)),
    # 4. Sequence test:<empty>+text:hello
    (Seq(text(""), text("hello")), 
        (("text", "hello"),)),
...
def test_basics(grammar: State, expected_results: Tuple[Tuple[str, str]]) -> None:
    outputs: List[StringDict] = list(grammar.generate())
    checkNumOutputs(outputs, len(expected_results))
    for tier, target in expected_results:
        checkHasOutput(outputs, tier, target)
